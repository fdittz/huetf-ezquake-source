...
static char *VX_SkinName(int player)
{
	...
	int bottomcolor = atoi(cl.players[player].bottomcolor);	
	char *team = cl.players[player].team;

	/*if (cl.playernum != player) {
		if (bottomcolor == atoi(cl.players[cl.playernum].bottomcolor)) {
			if (!(strcmp(cl.players[cl.playernum].team,team) == 0 )) {
				srand(time(NULL));
				player = ( rand() % MAX_CLIENTS );
			}
		}
	}*/
	...
	if (strstr(userskin, "tf_") != NULL) {
		for (int i = 0 ; i < NUMSKINFILES; i++) {
			if ((strcmp(skin_files[i].skinname, userskin) == 0))
				printf("%d\n",skin_files[i].bottomcolor);
				printf("%s\n",skin_files[i].skinfile);
		}
	}


	return Info_ValueForKey(cl.players[player].userinfo, "skin");
...
void VX_TrackerDeath(int player, int weapon, int count)
{
	...
	{
		snprintf(outstring, sizeof(outstring), "\\%s\\&c960You died&r\n%s deaths: %i", VX_SkinName(player), GetWeaponName(weapon), count);
	}
